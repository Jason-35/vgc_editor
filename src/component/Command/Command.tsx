...
function Command() {
  ...
  return (
    <div className="command fixed w-2/5 max-h-[50%] flex flex-col bg-orange-400 left-[50%] translate-x-[-50%] top-16 px-6 py-6 rounded-md">
        ...
        <nav className="list-none overflow-scroll command overflow-x-hidden overflow-y-auto">
          { Object.keys(commandList).map((command: string, index) => (
            ...
            className={`${command} command ${index === currentHover ? "bg-green-500" : ""} p-1 pr-4 hover:cursor-pointer hover:bg-green-300 flex list-none rounded-md pl-3`}>
            <span className={`${command} command inline-block w-2/3`}>{ command }</span>
            <span className={`${command} command text-right flex-1`}>{ commandList[command as keyof typeof commandList] as string }</span>
            </li>
          )) }
        </nav>
    </div>
